...
class PenaltyStageGate:
    ...
    def _stage_history(self, stage_key: Optional[str]) -> Optional[Deque[float]]:
        ...
        if history is None:
            history = deque(maxlen=max(self.success_early_death_window, 4))
    ...
    def handle_multi_eval(self, iteration: int, entry: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        stage_history = self._stage_history(stage_type_value) if stage_override_active else None
        if earlydeath is not None:
            ...
            if stage_history is not None:
                ...
                if len(stage_history) >= self.success_early_death_window:
                    stage_recent = list(stage_history)[-self.success_early_death_window :]
                    if stage_recent:
        ...
        if step_one_ok and earlydeath is not None:
            ...
            if earlydeath_ok and self.success_early_death_window > 1:
                median_candidate = stage_early_death_median if stage_override_active else early_death_median
