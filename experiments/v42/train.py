...
class PenaltyStageGate:
    ...
    def __init__(
        ...
        *,
        ...
        success_step_one_by_stage: Optional[Dict[str, int]] = None,
        ...
        self.success_step_one_by_stage = {}
        for key, value in (success_step_one_by_stage or {}).items():
            normalized = normalize_stage_type(key)
            if not normalized:
                continue
            self.success_step_one_by_stage[normalized] = max(int(value), 0)
    ...
    def _stage_step_one_threshold(self, stage_key: Optional[str]) -> int:
        normalized = normalize_stage_type(stage_key) if stage_key is not None else None
        if normalized and normalized in self.success_step_one_by_stage:
            return self.success_step_one_by_stage[normalized]
        return self.success_step_one
    ...
    def handle_multi_eval(self, iteration: int, entry: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        success = False
        step_one_ok = True
        stage_step_one_threshold = self._stage_step_one_threshold(stage_type_value)
        if stage_step_one_threshold >= 0:
            step_one_ok = step_one is not None and step_one <= stage_step_one_threshold
...
def train(args):
    ...
    success_step_one_overrides = parse_stage_int_overrides(
        args.penalty_gate_success_step_one_by_stage,
        "penalty_gate_success_step_one_by_stage",
        min_value=0,
    )
    ...
    if args.density_penalty_phase_targets:
        ...
        if penalty_gate_phase_allowance and args.penalty_gate_required_successes > 0:
            ...
            penalty_gate = PenaltyStageGate(
                ...
                success_step_one_by_stage=success_step_one_overrides or None,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="stage-aware step-one 阈值 (stage:value, 逗号分隔)，如 main:40",
    )
    parser.add_argument(
