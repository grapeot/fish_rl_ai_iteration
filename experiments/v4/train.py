...
def plot_stats(stats: Dict[str, List[float]], run_name: str, plot_path: Path):
    ...
    plt.ylabel("Value")
    plt.title(f"Fish RL dev_v4 - {run_name}")
    plt.legend()
...
def stats_gif(stats: Dict[str, List[float]], run_name: str, media_path: Path):
    ...
    for idx in range(1, len(iterations) + 1):
        ...
        ax1.set_title(f"dev_v4 progression ({run_name})")
        fig.tight_layout()
        fig.canvas.draw()
...
def evaluate_single_fish(model: PPO, num_fish: int, episodes: int = 5):
    results = []
...
def evaluate_multi_fish(model: PPO, num_fish: int, episodes: int = 3):
    ...
    for ep in range(episodes):
        ...
        while not done:
            actions = _predict_action(obs) if len(obs) > 0 else []
...
def train(args):
    ...
    run_name = args.run_name or datetime.now().strftime("dev_v4_%Y%m%d_%H%M%S")
    ...
    eval_results_single = evaluate_single_fish(model, num_fish=args.num_fish, episodes=args.eval_episodes)
    ...
    if args.eval_multi_fish and args.eval_multi_fish > 1:
        eval_multi = evaluate_multi_fish(model, num_fish=args.eval_multi_fish, episodes=args.eval_multi_episodes)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train PPO agent for Fish RL dev_v4")
