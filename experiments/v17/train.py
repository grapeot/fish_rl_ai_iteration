...
class AdaptiveBoostController:
    ...
    def update(self, iteration: int, p10_value: Optional[float]) -> Optional[Tuple[float, float]]:
        ...
        if target is None:
            return None
        if self._last_command is not None and abs(target - self._last_command) < 1e-6:
            return None
...
class IterationStatsCallback(BaseCallback):
    ...
    def _on_rollout_end(self) -> bool:
        ...
        first_death_samples = [
            float(ep.get("first_death_step"))
            for ep in episodes
            if ep.get("first_death_step") is not None and ep.get("first_death_step") >= 0
        ]
        first_death_p10 = float(np.percentile(first_death_samples, 10)) if first_death_samples else None
...
def plot_stats(stats: Dict[str, List[float]], run_name: str, plot_path: Path):
    ...
    plt.ylabel("Value")
    plt.title(f"Fish RL dev_v17 - {run_name}")
    plt.legend()
...
def stats_gif(stats: Dict[str, List[float]], run_name: str, media_path: Path):
    ...
    for idx in range(1, len(iterations) + 1):
        ...
        ax1.set_title(f"dev_v17 progression ({run_name})")
        fig.tight_layout()
        fig.canvas.draw()
...
def train(args):
    ...
    run_name = args.run_name or datetime.now().strftime("dev_v17_%Y%m%d_%H%M%S")
    ...
    for idx, (phase_num_fish, phase_iterations) in enumerate(curriculum):
        ...
        print(
            f"[dev_v17] Phase {idx + 1}/{len(curriculum)}: num_fish={phase_num_fish}, iterations={phase_iterations},"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train PPO agent for Fish RL dev_v17")
    ...
    parser.add_argument("--adaptive_boost_upper_p10", type=float, default=15.0, help="高于该 first_death_p10 时放宽 clamp")
    parser.add_argument("--adaptive_boost_low_speed", type=float, default=0.78, help="降速后的 escape boost 上限")
