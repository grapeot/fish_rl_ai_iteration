...
def plot_stats(stats: Dict[str, List[float]], run_name: str, plot_path: Path):
    ...
    plt.ylabel("Value")
    plt.title(f"Fish RL dev_v21 - {run_name}")
    plt.legend()
...
def stats_gif(stats: Dict[str, List[float]], run_name: str, media_path: Path):
    ...
    for idx in range(1, len(iterations) + 1):
        ...
        ax1.set_title(f"dev_v21 progression ({run_name})")
        fig.tight_layout()
        fig.canvas.draw()
...
def train(args):
    ...
    run_name = args.run_name or datetime.now().strftime("dev_v21_%Y%m%d_%H%M%S")
    ...
    for idx, (phase_num_fish, phase_iterations) in enumerate(curriculum):
        ...
        print(
            f"[dev_v21] Phase {idx + 1}/{len(curriculum)}: num_fish={phase_num_fish}, iterations={phase_iterations},"
    ...
    if args.eval_multi_fish and args.eval_multi_fish > 1 and callback.multi_eval_history:
        ...
        if candidate_iter is None or candidate_entry is None:
            ...
            if checkpoint_zip.exists():
                candidate_entry["best_model_path"] = str(checkpoint_zip)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train PPO agent for Fish RL dev_v21")
    ...
    parser.add_argument("--predator_pre_roll_steps", type=int, default=10, help="reset 后仅移动大鱼的预热步数，缓解 step=1 碰撞")
    parser.add_argument(
        "--predator_pre_roll_angle_jitter",
        type=float,
        default=0.3,
