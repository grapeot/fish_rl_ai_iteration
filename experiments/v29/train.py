...
class PenaltyStageGate:
    ...
    def handle_multi_eval(self, iteration: int, entry: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        success = False
        if (
            step_one is not None
            and step_one <= self.success_step_one
            and earlydeath is not None
            and earlydeath <= self.success_early_death
        ):
            p10_ok = True
            if self.success_first_death_p10 is not None:
                if median_window_value is None or median_window_value < self.success_first_death_p10:
                    p10_ok = False
            if p10_ok:
                success = True
        ...
        if failure and self.current_stage > 0 and self.failure_streak >= self.failure_tolerance:
...
class IterationStatsCallback(BaseCallback):
    ...
    def _collect_prewarm_usage(self) -> Optional[Dict[str, float]]:
        ...
        for entry in stats_list or []:
            ...
            cumulative_overrides += int(entry.get("total_overrides", 0) or 0)
        ratio = None
        if total_episodes > 0:
        ...
        return {
            ...
            "total_overrides": cumulative_overrides,
        }
    ...
    def _factory(rank: int):
        def _init():
            ...
            if prewarm_predator_velocity_seqs is not None and rank < len(prewarm_predator_velocity_seqs):
                seq = prewarm_predator_velocity_seqs[rank]
                if seq:
                    prepared = []
                    for item in seq:
...
def plot_stats(stats: Dict[str, List[float]], run_name: str, plot_path: Path):
    ...
    plt.ylabel("Value")
    plt.title(f"Fish RL dev_v26 - {run_name}")
    plt.legend()
...
def stats_gif(stats: Dict[str, List[float]], run_name: str, media_path: Path):
    ...
    for idx in range(1, len(iterations) + 1):
        ...
        ax1.set_title(f"dev_v26 progression ({run_name})")
        fig.tight_layout()
        fig.canvas.draw()
...
def train(args):
    ...
    if args.tail_seed_replay_path and args.tail_seed_prewarm_iterations > 0:
    ...
    run_name = args.run_name or datetime.now().strftime("dev_v26_%Y%m%d_%H%M%S")
    ...
    for idx, (phase_num_fish, phase_iterations) in enumerate(curriculum):
        ...
        print(
            f"[dev_v26] Phase {idx + 1}/{len(curriculum)}: num_fish={phase_num_fish}, iterations={phase_iterations},"
    ...
    export_tail_diagnostics(callback.step_one_records_all, run_name, tail_polar_plot_path, tail_summary_path)
    ...
    if args.eval_multi_fish and args.eval_multi_fish > 1 and callback.multi_eval_history:
        ...
        if candidate_iter is None or candidate_entry is None:
            ...
            if checkpoint_zip.exists():
                ...
                checkpoint_base = checkpoint_zip.with_suffix("")
    ...
    if tail_polar_plot_path.exists():
        print(f"Step-one heading polar: {tail_polar_plot_path}")
    if tail_summary_path.exists():
        print(f"Worst seeds summary: {tail_summary_path}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train PPO agent for Fish RL dev_v26")
    ...
    parser.add_argument(
        ...
        type=str,
        default="0.0,0.02,0.04,0.04,0.05,0.06,0.06",
