...
class PenaltyStageGate:
    ...
    def handle_multi_eval(self, iteration: int, entry: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if step_one_ok and earlydeath is not None and earlydeath <= self.success_early_death:
            p10_ok = True
...
class IterationStatsCallback(BaseCallback):
    ...
    def _collect_prewarm_usage(self) -> Optional[Dict[str, float]]:
        ...
        for entry in stats_list or []:
            ...
            forced_total += int(entry.get("forced_resets_total", 0) or 0)
        ratio = None
        if total_episodes > 0:
            ratio = float(total_overrides / max(total_episodes, 1))
        tail_stage = None
        if self.tail_stage_tracker:
            self.tail_stage_tracker.update_total_consumed(cumulative_overrides)
        ...
        return {
            ...
            "forced_resets_total": forced_total,
...
def parse_tail_seed_stage_spec(spec: str) -> List[Tuple[int, Path]]:
...
def train(args):
    ...
    if tail_stage_spec:
        ...
        for stage_idx, (stage_iterations, stage_path_raw) in enumerate(tail_stage_spec, start=1):
            ...
            if stage_velocities:
                ...
                tail_stage_metadata.append(
                    {
                        "label": f"stage{stage_idx}:{stage_path.stem}",
    ...
    if not tail_stage_metadata and tail_seed_sequences:
        ...
        if total_capacity > 0:
            tail_stage_metadata.append({"label": "prewarm", "capacity": total_capacity})
    ...
    run_name = args.run_name or datetime.now().strftime("dev_v32_%Y%m%d_%H%M%S")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="按阶段加载 tail seeds，格式 'iterations:path;iterations:path' (提供后覆盖传统单文件配置)",
    )
    parser.add_argument(
