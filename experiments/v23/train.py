...
class PenaltyStageGate:
    ...
    def handle_multi_eval(self, iteration: int, entry: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if failure and self.current_stage > 0:
...
def plot_stats(stats: Dict[str, List[float]], run_name: str, plot_path: Path):
    ...
    plt.ylabel("Value")
    plt.title(f"Fish RL dev_v22 - {run_name}")
    plt.legend()
...
def stats_gif(stats: Dict[str, List[float]], run_name: str, media_path: Path):
    ...
    for idx in range(1, len(iterations) + 1):
        ...
        ax1.set_title(f"dev_v22 progression ({run_name})")
        fig.tight_layout()
        fig.canvas.draw()
...
def train(args):
    ...
    run_name = args.run_name or datetime.now().strftime("dev_v22_%Y%m%d_%H%M%S")
    ...
    for idx, (phase_num_fish, phase_iterations) in enumerate(curriculum):
        ...
        print(
            f"[dev_v22] Phase {idx + 1}/{len(curriculum)}: num_fish={phase_num_fish}, iterations={phase_iterations},"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train PPO agent for Fish RL dev_v23")
    parser.add_argument("--total_iterations", type=int, default=60, help="迭代次数 (rollout count)")
    ...
    parser.add_argument("--eval_multi_episodes", type=int, default=24, help="多鱼 deterministic 评估 EP 数 (>=10 推荐)")
    ...
    parser.add_argument("--predator_spawn_jitter_radius", type=float, default=1.2, help="初始化时大鱼在圆心附近的随机半径 (世界坐标)")
    parser.add_argument("--predator_pre_roll_steps", type=int, default=12, help="reset 后仅移动大鱼的预热步数，缓解 step=1 碰撞")
    parser.add_argument(
        "--predator_pre_roll_angle_jitter",
        type=float,
        default=0.3,
