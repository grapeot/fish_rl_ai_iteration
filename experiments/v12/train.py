...
def plot_stats(stats: Dict[str, List[float]], run_name: str, plot_path: Path):
    ...
    plt.ylabel("Value")
    plt.title(f"Fish RL dev_v12 - {run_name}")
    plt.legend()
...
def stats_gif(stats: Dict[str, List[float]], run_name: str, media_path: Path):
    ...
    for idx in range(1, len(iterations) + 1):
        ...
        ax1.set_title(f"dev_v12 progression ({run_name})")
        fig.tight_layout()
        fig.canvas.draw()
...
def train(args):
    ...
    run_name = args.run_name or datetime.now().strftime("dev_v12_%Y%m%d_%H%M%S")
    ...
    for idx, (phase_num_fish, phase_iterations) in enumerate(curriculum):
        ...
        print(
            f"[dev_v12] Phase {idx + 1}/{len(curriculum)}: num_fish={phase_num_fish}, iterations={phase_iterations},"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train PPO agent for Fish RL dev_v12")
    ...
    parser.add_argument("--curriculum", type=str, default="15:6,20:9,25:4,25:5", help="逗号分隔的 num_fish:iterations 阶段定义，留空表示禁用")
