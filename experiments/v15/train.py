...
class IterationStatsCallback(BaseCallback):
    ...
    def _on_rollout_start(self) -> None:
        ...
        new_value = self.entropy_scheduler.value_for_iteration(next_iteration)
        if new_value is None:
            return
        self.current_entropy_coef = float(new_value)
        self.model.ent_coef = float(new_value)
        self._log(
            f"entropy_coef_update iteration={next_iteration:03d} value={self.current_entropy_coef:.4f}"
        )
    ...
    def _on_rollout_end(self) -> bool:
        ...
        first_death_step = self._extract_metric(episodes, "first_death_step", -1.0)
        ...
        boost_speed = self.current_escape_boost_speed
        if self.signals is not None:
            boost_speed = float(self.signals.escape_boost_speed)
            self.current_escape_boost_speed = boost_speed
        self.stats["escape_boost_speed"].append(boost_speed)
        ...
        self.logger.record("custom/escape_boost_speed", boost_speed)
        ...
        if self.density_penalty_scheduler is not None:
            new_penalty = self.density_penalty_scheduler.value_for_iteration(self.iteration)
            if new_penalty is not None:
                target = self.density_target
                if target is not None:
                    self.model.get_env().env_method("set_density_penalty", new_penalty, target)
                else:
                    self.model.get_env().env_method("set_density_penalty", new_penalty)
                self._log(f"density_penalty_update iteration={self.iteration:03d} value={new_penalty:.4f}")
            if self.density_penalty_scheduler.current_value is not None:
                self.current_density_penalty = float(self.density_penalty_scheduler.current_value)
...
def plot_stats(stats: Dict[str, List[float]], run_name: str, plot_path: Path):
    ...
    plt.ylabel("Value")
    plt.title(f"Fish RL dev_v15 - {run_name}")
    plt.legend()
...
def stats_gif(stats: Dict[str, List[float]], run_name: str, media_path: Path):
    ...
    for idx in range(1, len(iterations) + 1):
        ...
        ax1.set_title(f"dev_v15 progression ({run_name})")
        fig.tight_layout()
        fig.canvas.draw()
...
def train(args):
    ...
    run_name = args.run_name or datetime.now().strftime("dev_v15_%Y%m%d_%H%M%S")
    ...
    vec_env.env_method("set_escape_boost_speed", initial_escape_boost_speed)
    ...
    final_escape_boost_speed = initial_escape_boost_speed
    ...
    for idx, (phase_num_fish, phase_iterations) in enumerate(curriculum):
        ...
        final_escape_boost_speed = phase_escape_boost_speed
        if idx > 0:
            vec_env.close()
        ...
        vec_env.env_method("set_escape_boost_speed", phase_escape_boost_speed)
        phase_timesteps = phase_iterations * args.n_steps * args.num_envs
        print(
            f"[dev_v15] Phase {idx + 1}/{len(curriculum)}: num_fish={phase_num_fish}, iterations={phase_iterations},"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train PPO agent for Fish RL dev_v15")
    parser.add_argument("--total_iterations", type=int, default=26, help="迭代次数 (rollout count)")
    ...
    parser.add_argument(
        "--curriculum",
        type=str,
        default="15:6,20:7,25:4,25:4,25:3,25:2",
    ...
    parser.add_argument(
        ...
        type=str,
        default="3:0.78,4:0.8,5:0.82,6:0.82",
        help="按阶段覆盖 escape boost 速度，例如 '3:0.78,4:0.8,5:0.82,6:0.82'"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="0.0,0.02,0.04,0.05,0.06,0.08",
