...
class PenaltyStageGate:
    ...
    def handle_multi_eval(self, iteration: int, entry: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if step_one_ok and earlydeath is not None:
            earlydeath_ok = earlydeath <= self.success_early_death
            if earlydeath_ok and self.success_early_death_window > 1:
                if early_death_median is None or early_death_median > self.success_early_death:
                    earlydeath_ok = False
...
class IterationStatsCallback(BaseCallback):
    ...
    def _on_rollout_end(self) -> bool:
        ...
        warn_active, warn_since = self._update_tail_stage_warning(
            tail_queue_ratio,
            tail_stage_label,
            tail_queue_remaining,
        )
