...
def plot_stats(stats: Dict[str, List[float]], run_name: str, plot_path: Path):
    ...
    plt.ylabel("Value")
    plt.title(f"Fish RL dev_v16 - {run_name}")
    plt.legend()
...
def stats_gif(stats: Dict[str, List[float]], run_name: str, media_path: Path):
    ...
    for idx in range(1, len(iterations) + 1):
        ...
        ax1.set_title(f"dev_v16 progression ({run_name})")
        fig.tight_layout()
        fig.canvas.draw()
...
def train(args):
    ...
    run_name = args.run_name or datetime.now().strftime("dev_v16_%Y%m%d_%H%M%S")
    ...
    for idx, (phase_num_fish, phase_iterations) in enumerate(curriculum):
        ...
        print(
            f"[dev_v16] Phase {idx + 1}/{len(curriculum)}: num_fish={phase_num_fish}, iterations={phase_iterations},"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train PPO agent for Fish RL dev_v16")
    ...
    parser.add_argument(
        "--curriculum",
        type=str,
        default="15:6,20:7,25:4,25:4,25:5,25:4",
    ...
    parser.add_argument(
        ...
        type=str,
        default="3:0.76,4:0.78,5:0.82,6:0.82",
        help="按阶段覆盖 escape boost 速度，例如 '3:0.76,4:0.78,5:0.82,6:0.82'"
    )
    parser.add_argument(
        "--escape_boost_penalty_caps",
        type=str,
