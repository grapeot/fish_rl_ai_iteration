...
## 2025-11-11 更新
- `VectorizedFishEnv` 区分了实时存活数 (`num_alive`)、episode 平均 (`avg_num_alive`) 与终局 (`final_num_alive`)，并通过 `VecMonitor`/`CheckpointCallback` 全部写入日志与 TensorBoard（`tb_logs/`）。
...
- `PPO` 现在带 `tensorboard_log`，配合 `tensorboard --logdir tb_logs` 可实时查看 loss、熵以及新增自定义指标。

### 全量运行 (Iter=100, num_envs=32)
- 命令：`python train_v2.py --total_iterations 100 --num_envs 32`（env：`.venv`）。日志路径 `logs/train_v2_iter100.log`，快照+曲线归档于 `checkpoints/v2_iter100/`。
...
- 训练曲线：见 `checkpoints/v2_iter100/training_curve.png`；可观察到最高点在 Iter40 左右后即往下回落，推测策略在更高熵阶段短暂受益，但随后再次陷入同质化动作。
...
### 下一步计划（提案）
1. **实测 TensorBoard**：运行 `tensorboard --logdir tb_logs --port XXXX`，截图/记录 policy loss、entropy，验证是否存在熵坍缩或 value 爆炸（Checklist②）。
...
4. **版本分支**：完成上面 TODO 后创建 `train_v3.py` + `dev_v3.md`，并将新的 checkpoints 存在 `checkpoints/v3_*` 目录，遵循 SOP。
...
## 根因分析
1. **观测与动作错位**：`VectorizedFishEnv.step` (见 `train_v2.py:103-150`) 仍旧用同一个动作去控制所有存活小鱼，并且只返回第一条活鱼的观测。随着小鱼死亡，这个“第一条”不断换人，策略网络始终面对一个不连续、无法追踪身份的观测流，却承担着所有鱼的平均奖励，导致策略梯度几乎退化成噪声。（尚待在离线 TODO 中彻底解决，可考虑拆成单鱼 env）
...
## 下一步可验证的 Checklist
- [x] 修改环境后先运行 5 iteration，确认 `info` 中三类指标（当前、平均、终局）都能在日志里看到，且数值不同。（证据：`logs/train_v2_iter100.log` 多处包含 `num_alive`、`avg_num_alive`、`final_num_alive`）
